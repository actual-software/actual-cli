...
impl Orchestrator {
    ...
    /// Retries always continue indefinitely with backoff capped at
    /// `max_retry_backoff_ms` (§8.2 spec).
...
mod tests {
    ...
    async fn test_on_worker_exit_unlimited_retries() {
        // Retries always continue indefinitely (§8.2), even at high attempt counts
        let config = test_config();
        let orch = Orchestrator::new(config, "template".to_string());
