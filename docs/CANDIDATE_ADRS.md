# Candidate Generic ADRs: TypeScript / Next.js 16+

> Draft ADR content for the generic bank. Each ADR follows the schema and quality criteria from GOOD_ADRS.md. Organized by priority tier.

## Tier 1: Must-Have (Prevent Common Mistakes)

### ADR-001: Default to Server Components and Restrict `'use client'` to Leaf Interactive Components

```json
{
  "title": "Default to Server Components and Restrict 'use client' to Leaf Interactive Components",
  "context": "Next.js 15+ renders all components as Server Components by default. Adding 'use client' to a file makes it AND all its imports part of the client bundle, increasing JS sent to the browser. Misplacing the directive on layouts or parent components unnecessarily bloats the client.",
  "policies": [
...
### ADR-002: Return Error State from Server Actions Instead of Throwing

```json
{
  "title": "Return Error State from Server Actions Instead of Throwing",
  "context": "Server Actions handle form submissions and mutations. For expected errors (validation failure, API error), throwing exceptions triggers error boundaries which replace the form UI. Returning error state preserves the form and lets the user correct their input.",
  "policies": [
...
### ADR-003: Enable TypeScript Strict Mode and `noUncheckedIndexedAccess`

```json
{
  "title": "Enable TypeScript Strict Mode and noUncheckedIndexedAccess",
  "context": "TypeScript's strict mode catches null/undefined issues and enforces stronger function parameter checks. noUncheckedIndexedAccess (not included in strict) adds T | undefined to array/object index access, catching a common class of runtime TypeError crashes.",
  "policies": [
  ...
  "version": "TypeScript 4.1+ (noUncheckedIndexedAccess), TypeScript 2.0+ (strict)"
}
```

### ADR-004: Use `server-only` Package to Prevent Server Code Leaking to Client

```json
{
  "title": "Use server-only Package to Prevent Server Code Leaking to Client",
  "context": "Files with server secrets (API keys, database queries) can be accidentally imported into Client Components. Without protection, server-only env vars are silently replaced with empty strings on the client, causing silent failures instead of build errors.",
  "policies": [
...
### ADR-005: Prefix Client-Exposed Environment Variables with `NEXT_PUBLIC_`

```json
{
  "title": "Prefix Client-Exposed Environment Variables with NEXT_PUBLIC_",
  "context": "Next.js only inlines environment variables prefixed with NEXT_PUBLIC_ into the client JavaScript bundle at build time. Non-prefixed variables are replaced with empty strings on the client. This means NEXT_PUBLIC_ values are frozen at build time and visible to anyone inspecting the bundle.",
  "policies": [
...
### ADR-006: Call Revalidation Functions Before `redirect()` in Server Actions

```json
{
  "title": "Call Revalidation Functions Before redirect() in Server Actions",
  "context": "Next.js redirect() throws a framework-handled control-flow exception that stops all subsequent code execution. If revalidatePath() or revalidateTag() is called after redirect(), the cache will not be updated, leading to stale data on the destination page.",
  "policies": [
...
### ADR-007: Use `proxy.ts` Instead of `middleware.ts` in Next.js 16+

```json
{
  "title": "Use proxy.ts Instead of middleware.ts in Next.js 16+",
  "context": "Next.js 16 deprecated middleware.ts and renamed it to proxy.ts to clarify its purpose as a network-boundary proxy, not Express-style middleware. The function export changed from middleware() to proxy().",
  "policies": [
...
### ADR-008: Validate Server Action Inputs with Zod Schemas

```json
{
  "title": "Validate Server Action Inputs with Zod Schemas",
  "context": "FormData values are all strings and can be tampered with by the client. Server Actions are publicly accessible endpoints. Without validation, malformed or malicious input can cause data corruption or security issues.",
  "policies": [
  ...
  "category": "Security",
  "version": "Next.js 14+ / Zod 3+"
}
```

### ADR-009: Use Children Prop Pattern to Compose Server Components Inside Client Components

```json
{
  "title": "Use Children Prop Pattern to Compose Server Components Inside Client Components",
  "context": "Importing a Server Component file inside a 'use client' file makes it a Client Component (all imports in a client boundary become client code). To keep server-rendered content inside interactive client wrappers, pass Server Components as children props.",
  "policies": [
...
### ADR-010: Use `React.cache()` to Deduplicate Non-Fetch Data Access

```json
{
  "title": "Use React.cache() to Deduplicate Non-Fetch Data Access",
  "context": "React automatically memoizes fetch() GET requests within a render pass, but ORM calls, database queries, and CMS client calls are not memoized. Without React.cache(), calling getUser() in both a layout and a page results in two separate database queries for the same data.",
  "policies": [
...
### ADR-011: Fetch Data Directly in Server Components, Not Through Self-Referencing API Routes

```json
{
  "title": "Fetch Data Directly in Server Components, Not Through Self-Referencing API Routes",
  "context": "In the App Router, Server Components can directly query databases, call ORMs, and access APIs. Creating /api/ routes solely to fetch data for your own pages adds unnecessary network hops and complexity.",
  "policies": [
...
### ADR-012: Use Promise.all() for Parallel Data Fetching in Server Components

```json
{
  "title": "Use Promise.all() for Parallel Data Fetching in Server Components",
  "context": "Sequential await statements in Server Components block each other. If getArtist() takes 200ms and getAlbums() takes 300ms, sequential fetching takes 500ms while parallel fetching takes 300ms.",
  "policies": [
  ...
  "instructions": [
    ...
    "Use Promise.allSettled() instead if one failure shouldn't block the others"
  ],
...
### ADR-013: Use Suspense Boundaries with Skeleton Fallbacks for Streaming

```json
{
  "title": "Use Suspense Boundaries with Skeleton Fallbacks for Streaming",
  "context": "Without Suspense boundaries, slow data requests block the entire page from rendering. Suspense enables streaming: fast content renders immediately while slow content shows a fallback until ready.",
  "policies": [
  ...
  "category": "Performance",
  "version": "Next.js 13+ / React 18+"
}
```

### ADR-014: Use useActionState for Form State Management with Server Actions

```json
{
  "title": "Use useActionState for Form State Management with Server Actions",
  "context": "React 19 introduced useActionState to replace the pattern of useState + manual fetch + loading state for forms. It provides state, a form action function, and a pending boolean in one hook, with built-in progressive enhancement.",
  "policies": [
...
### ADR-015: Configure Proxy Matcher to Exclude Static Assets

```json
{
  "title": "Configure Proxy Matcher to Exclude Static Assets",
  "context": "Proxy (formerly middleware) runs on every matched request. Without a matcher, it processes static files, images, and internal Next.js routes unnecessarily, adding latency to every asset load.",
  "policies": [
...
### ADR-016: Use Route Groups for Domain-Based Layout Segmentation

```json
{
  "title": "Use Route Groups for Domain-Based Layout Segmentation",
  "context": "Different sections of an app often need different layouts (marketing vs dashboard, public vs authenticated). Route groups (parenthesized folders) let you apply different layouts without affecting URL structure.",
  "policies": [
...
### ADR-017: Use module Preserve for Next.js and Bundled TypeScript Projects

```json
{
  "title": "Use module Preserve for Next.js and Bundled TypeScript Projects",
  "context": "When using a bundler (Next.js, Vite, Webpack), TypeScript should not transform imports — the bundler handles module resolution. module: Preserve (implying moduleResolution: Bundler) tells TypeScript to leave imports as-is.",
  "policies": [
...
### ADR-018: Enable typedRoutes for Compile-Time Link Validation

```json
{
  "title": "Enable typedRoutes for Compile-Time Link Validation",
  "context": "Broken internal links are only discovered at runtime or in manual testing. Next.js can generate type definitions for all routes, making <Link href='...'> and router.push() type-checked at compile time.",
  "policies": [
  ...
  "category": "TypeScript Configuration",
  "version": "Next.js 13.2+"
}
```

### ADR-019: Use import type for Type-Only Imports with verbatimModuleSyntax

```json
{
  "title": "Use import type for Type-Only Imports with verbatimModuleSyntax",
  "context": "Regular import statements can include module side effects even when only types are used. Without import type, bundlers may not tree-shake type-only imports correctly, and module side effects execute unnecessarily.",
  "policies": [
...
### ADR-020: Understand the Next.js 4-Layer Cache Model

```json
{
  "title": "Understand the Next.js 4-Layer Cache Model",
  "context": "Next.js caches at four distinct layers with different lifetimes and locations. Adding custom caching without understanding these layers leads to stale data, over-caching, or redundant work.",
  "policies": [
