...
///
/// In TUI mode (`actual sync` without `--no-tui`), traces are written to a log
...
/// variable (e.g. `RUST_LOG=debug actual sync`).
fn init_logging(cli: &Cli) {
    use tracing_subscriber::EnvFilter;

    let filter = EnvFilter::try_from_default_env().unwrap_or_else(|_| EnvFilter::new("warn"));

    // Detect TUI mode: sync command with --no-tui not set.
    let is_tui = matches!(&cli.command, Command::Sync(a) if !a.no_tui);
