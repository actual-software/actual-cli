...
mod tests {
    ...
    // Test 47: is_likely_text_file returns true for empty files (no division by zero)
    #[test]
    fn test_is_likely_text_file_empty_file() {
        ...
        assert!(
            is_likely_text_file(&path),
            "empty files should be treated as text, not binary"
        );
    }
