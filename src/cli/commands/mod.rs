...
pub(crate) mod governance_session;
pub mod impl_check;
mod impl_check_hook;
pub mod login;
pub mod logout;
...
mod tests {
    ...
    fn test_handle_result_runner_failed_stderr_appears_in_rendered_panel() {
        // Verify that RunnerFailed with multi-line stderr returns exit code 1.
        // This exercises the stderr rendering path in handle_result that strips
        // ANSI codes and appends each line to the panel (commands/mod.rs:147-155).
        let code = handle_result(Err(ActualError::RunnerFailed {
            message: "Anthropic API returned error: 529".to_string(),
            stderr: "API overloaded — capacity exceeded\nPlease retry later.".to_string(),
        }));
        assert_eq!(code, 1);
    }
