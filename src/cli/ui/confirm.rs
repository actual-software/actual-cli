    ...
    if !project.languages.is_empty() {
        let langs: Vec<String> = project
            ...
            .map(|ls| {
                ...
                if let Some(ref sel) = project.selection {
                    if sel.language.language == ls.language {
                        // Selected: prefix with * as a plain-text bold indicator
                        format!("*{base}")
                    } else {
                        base
                    }
...
/// to present three options: Accept, Change, and Reject. This ensures plain-mode
/// users can trigger the Change action (re-select language/framework), which was
/// previously impossible with the binary `term.confirm` prompt.
...
pub fn prompt_project_confirmation(
    ...
    let options = vec![
        "Accept — proceed with sync".to_string(),
        "Change — re-select language/framework".to_string(),
        "Reject — abort".to_string(),
    ];
...
mod tests {
    ...
    //
    // After the switch from `term.confirm()` to `term.select_one()`, tests
    // use `.with_select_one(vec![index])` where 0=Accept, 1=Change, 2=Reject.
    ...
    fn prompt_change() {
        let analysis = make_single_project_analysis();
        let term = MockTerminal::new(vec![]).with_select_one(vec![1]); // 1 = Change
        let result = prompt_project_confirmation(&analysis, &term).unwrap();
    ...
    fn prompt_out_of_range_defaults_to_reject() {
        let analysis = make_single_project_analysis();
        // Any index >= 2 maps to Reject via the wildcard match arm
    ...
    #[test]
    fn prompt_user_cancelled_propagates_error() {
        let analysis = make_single_project_analysis();
        // Empty select_one results causes UserCancelled error
        let term = MockTerminal::new(vec![]).with_select_one(vec![]);
        let result = prompt_project_confirmation(&analysis, &term);
        assert!(
            result.is_err(),
            "expected Err when terminal select_one fails, got: {result:?}"
        );
    }
    ...
    fn format_metadata_themed_auto_selected_no_text_badge() {
        ...
        // No text badges — styling only (bold+accent vs dim)
        assert!(
            !plain.contains("(auto)"),
        ...
        // Selected and non-selected frameworks should appear
        assert!(
            plain.contains("actix-web"),
    ...
    fn format_metadata_themed_no_selection_unchanged() {
        ...
        let joined = lines.join("\n");
        let plain = strip(&joined);
        // No selection markers of any kind
        assert!(
            !plain.contains("(auto)"),
    ...
    #[test]
    fn format_metadata_plain_auto_selected_shows_star_prefix() {
        let project = make_project_with_selection(true);
        let lines = format_metadata_lines_plain(&project);
        let joined = lines.join("\n");
        // Selected language should have * prefix
        assert!(
        ...
        assert!(
            ...
            "should not contain [auto] text badge in: {joined}"
        );
        assert!(
            !joined.contains("[selected]"),
            "should not contain [selected] text badge in: {joined}"
        );
    ...
    #[test]
    fn format_metadata_plain_user_selected_shows_star_prefix() {
        let project = make_project_with_selection(false);
        let lines = format_metadata_lines_plain(&project);
        let joined = lines.join("\n");
        // Selected language should have * prefix
    ...
    #[test]
    fn format_metadata_plain_no_selection_no_star_prefix() {
        let project = Project {
            path: ".".to_string(),
            name: "test".to_string(),
            languages: vec![LanguageStat {
        ...
        assert!(
            ...
            "should not contain * prefix with no selection in: {joined}"
        );
        assert!(
            !joined.contains("*actix-web"),
            "should not contain * prefix with no selection in: {joined}"
