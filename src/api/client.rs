...
/// Default base URL for the Advisor API. Advisor traffic routes through the
/// mcp-gateway (Fork A): the gateway validates the CLI's RS256 OAuth token and
/// forwards the request to api-service with a ServiceKey + `X-Org-Id` +
/// `X-Integration-Source: cli`, rather than the CLI hitting api-service
/// directly. Only the base host differs from [`DEFAULT_API_URL`] — the gateway
/// mirrors the advisor route paths (`/v1/advisor/query`,
/// `/v1/advisor/query/{id}`), so non-advisor calls (`/adrs/match`, taxonomy,
/// health) still use [`DEFAULT_API_URL`].
pub const DEFAULT_ADVISOR_API_URL: &str = "https://mcp.actual.ai";
...
impl ActualApiClient {
    ...
    pub async fn start_advisor_query(
        ...
        // A 403 from the advisor gateway means the session's org is not
        // authorized for the request (cross-org, fail-closed). Surface it as a
        // distinct, actionable error rather than an opaque API error.
    ...
    /// The default cross-organization error for a `403` from the advisor
    /// gateway. The advisor command layer replaces this with a message naming
...
mod tests {
    ...
    #[test]
    fn test_default_advisor_api_url() {
        // Advisor traffic defaults to the mcp-gateway, not api-service.
        assert_eq!(DEFAULT_ADVISOR_API_URL, "https://mcp.actual.ai");
        assert_ne!(DEFAULT_ADVISOR_API_URL, DEFAULT_API_URL);
    }

    #[tokio::test]
    async fn test_start_advisor_query_forbidden_maps_to_org_mismatch() {
        let mut server = mockito::Server::new_async().await;
        // The gateway fail-closes a cross-org token with 403 on the start call.
        let mock = server
            .mock("POST", "/v1/advisor/query")
            .with_status(403)
            .with_header("content-type", "application/json")
