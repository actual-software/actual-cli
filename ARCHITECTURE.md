...
- ADR `29aaf503` (`.claude/rules/`) requires a `benches/` directory that does
  not exist, so its own verify command (`test -d benches`) fails today. This is
  a stale artifact, not a TODO: the source ADR cites
  `benches/analysis_baseline.rs` as an existing file, and `Cargo.toml` still
  carries `autobenches = false` while `.gitignore` still ignores
  `benches/*_results*.json` — leftovers from a benchmark suite the repo no
  longer has. Don't add `benches/` just to satisfy the rule, and don't hand-edit
  `.claude/rules/` to drop it; those files are regenerated from `docs/adr/`.
  Check the repo before treating any rule's accept criteria as ground truth.
